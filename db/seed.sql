...
-- now tracks subcategory directly (no more 3-bucket
-- SUBCATEGORY_TO_SERVICE_CATEGORY grouping) — set equal to subcategory on
-- every row below. product_type ('OEM'|'Aftermarket'|'New'|'Second Hand')
-- is populated here so local dev/TESTING.md has non-null data to exercise
-- the part-type search filter.
-- ============================================================
INSERT INTO products (name, brand, reference, synonyms, category_keywords, category, subcategory, service_category, product_type, description)
VALUES
  ('Mann Oil Filter W712/75',    'Mann',       'W712/75',     'oil filter lubricant filter', 'oil filter engine',          'lubricant', 'Filtration', 'Filtration', 'OEM',         'Mann Oil Filter W712/75'),
  ('Bosch Oil Filter P7153',     'Bosch',      'P7153',       'oil filter lubricant filter', 'oil filter engine',          'lubricant', 'Filtration', 'Filtration', 'Aftermarket', 'Bosch Oil Filter P7153'),
  ('Mahle Oil Filter OC611',     'Mahle',      'OC611',       'oil filter original oem',     'oil filter engine',          'lubricant', 'Filtration', 'Filtration', 'OEM',         'Mahle Oil Filter OC611'),
  ('Original VW Oil Filter',     'VW OEM',     '06J115403Q',  'oil filter original oem',     'oil filter engine',          'lubricant', 'Filtration', 'Filtration', 'OEM',         'Original VW Oil Filter'),
  ('Hilux Front Shock Absorber', 'KYB',        'KYB334816',   'shock absorber front strut',  'suspension shock absorber',  'part',      'Suspension', 'Suspension', 'Aftermarket', 'Hilux Front Shock Absorber'),
  ('Golf Front Brake Pads',      'Textar',     'TEX2369201',  'brake pads brake shoes',      'brakes brake pads',          'part',      'Brakes',     'Brakes',     'Aftermarket', 'Golf Front Brake Pads'),
  ('Mann Oil Filter W712/75 Kit','Mann',       'W712/75-KIT', 'oil filter lubricant filter', 'oil filter engine',          'lubricant', 'Filtration', 'Filtration', 'OEM',         'Mann Oil Filter W712/75 Kit'),
  ('Continental Timing Belt CT1028', 'Continental', 'CT1028',  'timing belt distribution belt','timing belt engine',       'part',      'Engine',     'Engine',     'New',         'Continental Timing Belt CT1028'),
  ('Fram Air Filter CA1234',     'Fram',       'CA1234',      'air filter engine filter',     'air filter engine',         'part',      'Filtration', 'Filtration', 'Aftermarket', 'Fram Air Filter CA1234')
...
VALUES
  ...
  -- (Filtration), to exercise the WhatsApp service-matching follow-up (see
  -- CLAUDE.md message pipeline / product.service.ts's
  -- processBasketConfirmation -> getMatchingServicesForProduct).
...
-- spanning the products.subcategory vocabulary (Filtration/Brakes/Engine),
-- so local dev/TESTING.md has something to exercise the direct
-- products.service_category <-> services.service_category join against (see
-- db/schema.sql services table comment). service_category values here match
-- products.subcategory directly — no more maintenance/general_mechanics/
-- diagnostics 3-bucket scheme. "Computerized Electronic Diagnostics" has no
-- diagnostics-specific product subcategory today, so it's reassigned to
-- 'Engine' (the closest existing subcategory) rather than left orphaned.
-- ============================================================
...
VALUES
  ((SELECT id FROM service_providers WHERE name = 'Auto Drex'), 'Engine Oil and Filter Change (Oil, Air, AC) - Petrol Engines', 'Filtration', 15000, 1.0, true, 5000, 'Zone 1 (Talatona/Kilamba/Benfica): 5000 Kz | Zone 2 (Maianga/Alvalade/Viana): 8000 Kz | Zone 3 (Baixa/Mutamba/Cacuaco): 12000 Kz'),
  ((SELECT id FROM service_providers WHERE name = 'Auto Drex'), 'Front Brake Pad Replacement', 'Brakes', 15000, 1.0, true, 5000, 'Zone 1 (Talatona/Kilamba/Benfica): 5000 Kz | Zone 2 (Maianga/Alvalade/Viana): 8000 Kz | Zone 3 (Baixa/Mutamba/Cacuaco): 12000 Kz'),
  ((SELECT id FROM service_providers WHERE name = 'Auto Drex'), 'Computerized Electronic Diagnostics (Scanner)', 'Engine', 15000, 0.5, true, 5000, 'Zone 1 (Talatona/Kilamba/Benfica): 5000 Kz | Zone 2 (Maianga/Alvalade/Viana): 8000 Kz | Zone 3 (Baixa/Mutamba/Cacuaco): 12000 Kz')
